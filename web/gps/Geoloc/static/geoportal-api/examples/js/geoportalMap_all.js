...
function initMap() {
    ...
    var language= Args.l || OpenLayers.Lang.getCode();
    if (!OpenLayers.Lang[language]) { language= 'fr'; }
    var re= new RegExp("^"+language);
    OpenLayers.Lang.setCode(language);
    var territory= Args.t || 'FXX';
    if (!Geoportal.Catalogue.TERRITORIES[territory]) { territory= 'FXX'; }
    ...
    var lsc= viewer.getMap().getControlsByClass('Geoportal.Control.LayerSwitcher')[0];
    var ster= new Geoportal.Control.TerritorySelector({
        div: lsc.createControlAnchor(lsc.id+'_ster','gpControlTerritorySelector'),
        territory:territory,
        autoActivate:true
    });
    ...
    viewer.getMap().setCenter(
        viewer.getMap().catalogue.getCenter(territory,viewer.getMap().getProjection()),//center in current projection
        viewer.getMap().catalogue.getDefaultZoom(territory),
        false,
        true
    );
